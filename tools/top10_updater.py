...
the LAST 25 MATCHES per player, all of them pro (MatchType 2), wins AND losses. Built
2026-08-29 from three suggestions by pro player GG Qwaser. The deep pass runs after the
...
def run_deep():
    ...
    for rank, (pid, rating) in enumerate(top, 1):
        ...
        rows.append({"r": rank, "n": name, "s": rating, "mp": w + l, "w": w, "l": l,
                     "k": K, "d": D, "kd": round(K / float(D), 2) if D else None,
                     "heroes": heroes[:5], "mates": mates})
